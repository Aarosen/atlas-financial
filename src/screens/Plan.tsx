...
export function PlanScreen({
  ...
  const leverLabelMap: Record<string, string> = {
    ...
    build_emergency_buffer: 'Build emergency buffer',
    increase_future_allocation: 'Increase future allocation',
    optimize_discretionary_spend: 'Optimize discretionary spend',
  };
  const leverLabel = leverLabelMap[baseline.lever] || baseline.lever;
  const primaryStep = baseline.explainability?.nextAction?.prompt || 'Take one grounded step this week.';
  ...
  return (
    <ScreenWrap>
      ...
      <PageContainer maxWidth={820} style={{ paddingTop: 'var(--padY)', paddingBottom: 'var(--padY)' }}>
        <Stack gap={14}>
          <Card>
            <div style={{ fontWeight: 900, fontSize: 12, letterSpacing: '0.08em', color: 'var(--ink2)' }}>LEVER</div>
            <div style={{ marginTop: 6, fontWeight: 980, fontSize: 20 }}>{leverLabel}</div>
            <div style={{ marginTop: 10, color: 'var(--ink2)', lineHeight: 1.7 }}>One move that matters most right now.</div>
          </Card>

          <Card>
            <div style={{ fontWeight: 900, fontSize: 12, letterSpacing: '0.08em', color: 'var(--ink2)' }}>PRIMARY STEP</div>
